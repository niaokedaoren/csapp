 ...
 *
 * lazy rusher, weichen1@andrew.cmu.edu
 */
#include <assert.h>
#include <stdio.h>
#include <stdlib.h>
...
pid_t waiting_pid; /* the pid of foreground job, -1 if no foreground job */
...
void run_buildin(struct cmdline_tokens *cmd, int in, int out);
...
eval(char *cmdline)
{
    ...
    if (tok.builtins != BUILTIN_NONE) { /* deal with build in commands */
        int f1 = 0, f2 = 1; /* default io file descriptor */
        if (tok.infile != NULL)
            f1 = Open(tok.infile, O_RDONLY, 0); /* redirect for stdin */

        if (tok.outfile != NULL)
            f2 = Open(tok.outfile, O_RDWR, 0); /* redirect for stdout */

        run_buildin(&tok, f1, f2);
    } else { /* run external executable file */
        int pid;
        ...
        if ((pid = Fork()) == 0) { /* child job */
            /* redirect IO */
            if (tok.infile != NULL) {
                int f1 = Open(tok.infile, O_RDONLY, 0);
                dup2(f1, 0);
            }

            if (tok.outfile != NULL) {
                int f2 = Open(tok.outfile, O_RDWR, 0);
                dup2(f2, 1);
            }
        ...
        /* wait for foreground job to end */
        if (!bg) {
            Wait();
        }
    }
...
/*
 * run buildin commands, namely: jobs, bg, fg, quit
 *
 * cmd: the command line token we parsed
 * in: read in file descriptor
 * out: write out file descriptor
 */
void run_buildin(struct cmdline_tokens *cmd, int in, int out) {
    char *jidc;
    int jid;
    ...
    case BUILTIN_BG: /* bg */
        ...
        Kill(-job->pid, SIGCONT); /* continue bg's process group */
    ...
    case BUILTIN_FG: /* fg */
        ...
        job = getjobjid(job_list, jid);
        waiting_pid = job->pid;
        Kill(-job->pid, SIGCONT); /* continue fg's process group */
        job->state = FG;
        Wait();
        break;
    default:
...
/*
 * create a child, wrapper for fork.
 */
pid_t Fork(void) {
    pid_t pid;
...
/*
 * Open a file, wrapper for open
 *
 * pathname: file name
 * flags: read or write flag
...
/*
 * block a particular signal for current process
 *
 * sig: signal number
 */
void Block(int sig) {
...
/*
 * unblock a particular signal for current process
 *
 * sig: signal number
 */
void Unblock(int sig) {
...
/*
 * wait for the foreground job to end
 */
void Wait(void){
    sigset_t mask;
